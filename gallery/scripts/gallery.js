...
document.body.insertAdjacentHTML("beforeend", '<div id="js-viewer"><img id="js-viewer-img" src="" /><span><button id="js-viewer-prev" type="button">&laquo;</button> <button id="js-viewer-next" type="button">&raquo;</button></span></div>');
...
document.addEventListener("click", (e) => {
    ...
    if (e.target.parentElement != undefined && e.target.parentElement.tagName == "A" && e.target.parentElement.parentElement != undefined && e.target.parentElement.parentElement.parentElement != undefined && e.target.parentElement.parentElement.parentElement.className == "photos") {
        e.preventDefault();
        viewPhoto(e.target.parentElement.parentElement);
    }
    if (e.target.tagName == "A" && e.target.parentElement.className == "photos") {
        e.preventDefault();
        viewPhoto(e.target);
    }
    // Viewer Hide!
